...
def test_probe_sends_curl_user_agent():
    """Hosted STT (Groq) sits behind Cloudflare, which 403s Python-urllib's default UA.
    The probe must look like curl so a valid token is not classified unauthorized."""
    ...
    assert srv.user_agents == ["curl/8.7.1"]
