"""Structured, trace-correlated logging for meeting-api — conforms to ``logevent.v1``.
...
gateway and runtime services, each bound to its own ``SERVICE`` name; the wire format is
...
(reuse an incoming id; mint only if absent) so this hop's logs share the caller's trace_id.
"""
...
class TraceMiddleware(BaseHTTPMiddleware):
    """Read ``X-Trace-Id`` (reuse an incoming id from the upstream gateway; mint only if
