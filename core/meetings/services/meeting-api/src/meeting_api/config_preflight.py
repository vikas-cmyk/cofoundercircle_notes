...
def _http_probe(spec: dict, env: Mapping[str, str], timeout: float) -> dict:
    ...
    # Python-urllib's default UA is blocked by Cloudflare (error 1010 → HTTP 403) in front of
    # Groq and other hosted STT. curl's UA is accepted; the bot's Node fetch is too. Probe with
    # the curl UA so a working Groq token is not classified as `unauthorized`.
