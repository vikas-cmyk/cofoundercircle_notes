#!/usr/bin/env node
/**
 * gate:schema for webhook.v1 — validate every golden against webhook.schema.json.
 * Convention: a golden filename is `<Shape>.<case>.json`; the part before the first dot is the
 * `$def` it must conform to (e.g. `Envelope.completed.json` → #/$defs/Envelope).
...
console.log(failed ? `webhook.v1: ${failed} golden(s) FAILED` : `webhook.v1: ${files.length} goldens conform`);
