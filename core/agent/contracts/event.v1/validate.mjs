#!/usr/bin/env node
/** gate:schema for event.v1 — golden `<Shape>.<case>.json` validates against #/$defs/<Shape>. */
...
console.log(failed ? `event.v1: ${failed} golden(s) FAILED` : `event.v1: ${files.length} goldens conform`);
